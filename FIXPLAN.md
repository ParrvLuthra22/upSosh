...
**Status: Part 1 done, Part 2 "delete half-built surfaces" done.** Part 1 (P2-1 through P2-5, P2-7, P2-8 partial, P2-9) closed the `src/` vs root split entirely (`frontend/src/` no longer exists) and the two-API-client bug. Part 2's breadth pass (see its own dated entry below) deleted 9 half-built routes including P2-6, deleted the 3 remaining mock-data files, and removed or fixed the 10 dead-UI items tracked in P4-20. **Still open**: the design-system unification (P2-11→18) and the remaining shared-logic consolidation (P2-20→28) — that's a third pass. P2-19 (EventCard / booking-store duplicate pairs) is *explicitly deferred*, not just unstarted — see its note.
