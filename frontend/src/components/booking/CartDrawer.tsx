...
const CartDrawer = () => {
    ...
    return (
        ...
        >
            ...
            <div
                ref={drawerRef}
                className="absolute top-0 right-0 h-full w-full max-w-md bg-surface border-l border-white/10 shadow-2xl transform translate-x-full pointer-events-auto flex flex-col"
            >
                {/* Header */}
                <div className="p-6 border-b border-border/50 flex items-center justify-between">
                    <h2 id="cart-title" className="text-2xl font-heading font-bold text-text-primary">Your Cart</h2>
                    <button
                        onClick={() => toggleCart(false)}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6" aria-live="polite">
                    {cart.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center text-text-muted">
                            ...
                            <button
                                onClick={() => toggleCart(false)}
                                className="mt-4 text-primary hover:underline focus:outline-none focus:ring-2 focus:ring-primary rounded-md px-2"
                            >
                                Browse Events
                            </button>
                        </div>
                        ...
                        cart.map((item) => (
                            <div key={item.id} className="flex gap-4">
                                ...
                                <div className="flex-1 flex flex-col justify-between">
                                    <div>
                                        <h3 className="font-bold text-text-primary line-clamp-1">{item.title}</h3>
                                        <p className="text-sm text-text-muted">{item.date}</p>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 bg-surface-highlight rounded-md p-0.5">
                                            <button
                                                onClick={() => updateCartQty(item.id, item.qty - 1)}
                                                className="w-6 h-6 flex items-center justify-center rounded hover:bg-surface text-xs focus:outline-none focus:ring-2 focus:ring-primary"
                                                aria-label={`Decrease quantity of ${item.title}`}
                                            >
                                                -
                                            </button>
                                            <span className="text-xs font-medium w-3 text-center" aria-label="Quantity">{item.qty}</span>
                                            <button
                                                onClick={() => updateCartQty(item.id, item.qty + 1)}
                                                className="w-6 h-6 flex items-center justify-center rounded hover:bg-surface text-xs focus:outline-none focus:ring-2 focus:ring-primary"
                ...
                {cart.length > 0 && (
                    <div className="p-6 border-t border-border/50 bg-surface/50 backdrop-blur-md">
                        ...
                        <button
                            ...
                            className="w-full py-4 rounded-xl bg-primary text-white font-bold text-lg hover:opacity-90 transition-opacity shadow-lg shadow-primary/20 focus:outline-none focus:ring-2 focus:ring-primary"
                        >
                            Checkout
                        </button>
                    </div>
